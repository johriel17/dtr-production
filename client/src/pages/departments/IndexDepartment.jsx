...
const IndexDepartment = () => {
    ...
    const getDepartments = async (page, searchQuery) => {
        try {
        setLoading(true);
        
        const res = await api.get(`/departments?page=${page}&search=${searchQuery}`);
    ...
    useEffect(()=>{ 
        ...
        getDepartments(currentPage, searchQuery);
    ...
    const handleDelete = (department) =>{ 
        setRemovedDtr(department)
        SetShowDeleteModal(true)
    }

    const handleConfirmDelete = async() =>{
        try{
            await api.delete(`/departments/${removedDepartment._id}`)
  ...
  return (
    <div className="px-10 bg-slate-200 rounded-lg shadow-lg my-10">
        <div className="flex justify-between items-center">
            <h1 className="text-3xl my-8 ">Departments List</h1>
            ...
            <table className='w-full border-separate border-spacing-2 py-5'>
                <thead>
                    <tr>
                        ...
                        <th className='border border-slate-600 rounded-md'>Department Name</th>
                        ...
                        <tr key={department._id} className='h-8'>
                            ...
                            <td className='border border-slate-700 rounded-md text-center'>
                                <div className='flex justify-center gap-x-4'>
                                    <Link to={`/departments/view/${department._id}`}>
        ...
        toDelete={removedDepartment.name}
      />
    </div>
  )
}
